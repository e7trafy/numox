...
<div class="col-md-6">
    <div class="mb-1">
        <label class="form-label">{{__('siteTrans.price')}}</label>
...
<div class="col-md-6">
    <div class="mb-1"><label for="validationCustom04" class="form-label">{{__('siteTrans.period')}}</label>
        <select class="form-select" id="validationCustom04" name="period" {{ Route::is('*.create') ? 'required' : '' }}>
            <option selected disabled value="">Choose...</option>
            <option value="yearly"
                {{isset($row) && $row->period == 'yearly' ? 'selected' : ''}}>
                {{__('siteTrans.yearly')}}
            </option>
            <option value="monthly"
                {{isset($row) && $row->period == 'monthly' ? 'selected' : ''}}>
                {{__('siteTrans.monthly')}}
            </option>
        </select>
        @include('dashboard.layouts.shared.validationStatus')
    </div>
